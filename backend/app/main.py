...
def configured_public_host() -> str | None:
    ...
    try:
        is_ip_address = ipaddress.ip_address(hostname)
    except ValueError:
        is_ip_address = None
    ...
    if (port is not None or hostname in LOOPBACK_HOSTS or "*" in hostname
            or (is_ip_address is None and not valid_dns_name)):
        return None
    return hostname
